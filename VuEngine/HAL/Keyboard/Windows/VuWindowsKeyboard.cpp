...
// the interface
IMPLEMENT_SYSTEM_COMPONENT(VuKeyboard, VuWindowsKeyboard);


//*****************************************************************************
VuWindowsKeyboard::VuWindowsKeyboard()
{
	memset(mKeyMap, 0, sizeof(mKeyMap));

	// register event handlers
	REG_EVENT_HANDLER(VuWindowsKeyboard, OnWindowsKeyDown);
	REG_EVENT_HANDLER(VuWindowsKeyboard, OnWindowsKeyUp);
}

//*****************************************************************************
bool VuWindowsKeyboard::init()
{
	if ( !VuKeyboard::init() )
		return false;
...
//*****************************************************************************
void VuWindowsKeyboard::onKeyDown(VUUINT virtualKey)
{
	VUUINT32 key = mKeyMap[virtualKey];
	if ( key )
	{
		VuParams params;
		params.addUnsignedInt(key);
		VuEventManager::IF()->broadcastDelayed(0.0f, true, "OnWindowsKeyDown", params);
	}
}

//*****************************************************************************
void VuWindowsKeyboard::onKeyUp(VUUINT virtualKey)
{
	VUUINT32 key = mKeyMap[virtualKey];
	if ( key )
	{
		VuParams params;
		params.addUnsignedInt(key);
		VuEventManager::IF()->broadcastDelayed(0.0f, true, "OnWindowsKeyUp", params);
	}
}

//*****************************************************************************
void VuWindowsKeyboard::OnWindowsKeyDown(const VuParams &params)
{
	VuParams::VuAccessor accessor(params);
	VUUINT32 key = accessor.getUnsignedInt();

	onKeyDownInternal(key);
}

//*****************************************************************************
void VuWindowsKeyboard::OnWindowsKeyUp(const VuParams &params)
{
	VuParams::VuAccessor accessor(params);
	VUUINT32 key = accessor.getUnsignedInt();
