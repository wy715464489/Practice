...
protected:
	...
	virtual	void*	findPointer(void* oldPtr) 
	{
		...
		void	writeDNA()
		{
			...
			VU_MEMCPY(dnaChunk->m_oldPtr,m_dnaLength,m_dna,m_dnaLength);
		...
		void initDNA(const char* bdnaOrg,int dnalen)
		{
			...
			VU_MEMCPY(m_dna,dnalen,bdnaOrg,dnalen);
			m_dnaLength = dnalen;
...
public:	
		...
		void	writeHeader(unsigned char* buffer) const
		{
			...
			VU_MEMCPY(buffer, 7, "BULLETd", 7);
#else
			VU_MEMCPY(buffer, 7, "BULLETf", 7);
		...
		virtual	void	finishSerialization()
		{
			...
			if (!m_totalSize)
			{
				...
				for (int i=0;i<	m_chunkPtrs.size();i++)
				{
					...
					VU_MEMCPY(currentPtr,curLength, m_chunkPtrs[i], curLength);
