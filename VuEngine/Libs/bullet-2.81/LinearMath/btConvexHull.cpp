...
//		if ( 1 ) // scale vertices back to their original size.
		...
		if ( ok )
		{
			...
			if ( desc.HasHullFlag(QF_TRIANGLES) ) // if he wants the results as triangle!
			{
				...
				VU_MEMCPY(&result.m_OutputVertices[0], sizeof(btVector3)*ovcount, &vertexScratch[0], sizeof(btVector3)*ovcount );
  			...
  			if ( desc.HasHullFlag(QF_REVERSE_ORDER) )
				...
				{
					VU_MEMCPY(&result.m_Indices[0], sizeof(unsigned int)*hr.mIndexCount, &hr.m_Indices[0], sizeof(unsigned int)*hr.mIndexCount);
				}
			}
			else
			{
