  ...
  while (destLenOrig > 0)
  {
    if (p->bufPos != p->bufConv)
    {
      ...
      if (curSize > destLenOrig)
        curSize = destLenOrig;
      VU_MEMCPY_7Z(dest, p->buf + p->bufPos, curSize);
      p->bufPos += curSize;
      *destLen += curSize;
      dest += curSize;
      destLenOrig -= curSize;
    ...
    p->bufConv = 0;
    {
      ...
      if (curSize > srcLenOrig)
        curSize = srcLenOrig;
      VU_MEMCPY_7Z(p->buf + p->bufTotal, src, curSize);
      *srcLen += curSize;
      src += curSize;
      srcLenOrig -= curSize;
      p->bufTotal += curSize;
...
SRes XzBlock_Parse(CXzBlock *p, const Byte *header)
{
  ...
  for (i = 0; i < numFilters; i++)
  {
    ...
    VU_MEMCPY_7Z(filter->props, header + pos, (size_t)size);
    pos += (unsigned)size;
  ...
  *status = CODER_STATUS_NOT_SPECIFIED;
  for (;;)
  {
    ...
    if (p->state == XZ_STATE_BLOCK)
    {
	  ...
	  ECoderFinishMode fm = (finishMode == CODER_FINISH_ANY) ? CODER_FINISH_ANY : CODER_FINISH_END;
    ...
    switch(p->state)
    {
      ...
      case XZ_STATE_BLOCK_HEADER:
      {
        ...
        else if (p->pos != p->blockHeaderSize)
        {
          ...
          if (cur > srcRem)
            cur = (UInt32)srcRem;
          VU_MEMCPY_7Z(p->buf + p->pos, src, cur);
          p->pos += cur;
          (*srcLen) += cur;
          src += cur;
        }
