...
#include "VuVertexCacheOpt.h"

#if VU_DISABLE_VERTEX_CACHE_OPT

void Forsyth::OptimizeFaces(const uint16* indexList, uint indexCount, uint vertexCount, uint16* newIndexList, uint16 lruCacheSize)
{
	VUASSERT(0, "vertex cache optimization not enabled");
}
...
namespace Forsyth
{
    namespace
    {
        ...
        float ComputeVertexCacheScore(int cachePosition, uint vertexCacheSize)
        {
            ...
            else
            {
                ...
                {
                    VUASSERT(cachePosition < (int)vertexCacheSize, "Forsyth");
        ...
        {
            VUASSERT(s_vertexScoresComputed, "Forsyth");
    ...
    {
		(void)&FindVertexCacheScore;		// done for warning suppression about no calls to FindVertexCacheScore()
		(void)&FindVertexValenceScore;	// "
        ...
        for (uint i=0; i<indexCount; ++i)
        {
            ...
            VUASSERT(index < vertexCount, "Forsyth");
        ...
        for (uint i = 0; i < indexCount; i += 3)
        {
            if (bestScore < 0.f)
            {
                ...
                for (uint j = 0; j < indexCount; j += 3)
                {
                    if (processedFaceList[j] == 0)
                    {
                        ...
                        for (uint k=0; k<3; ++k)
                        {
                            ...
                            OptimizeVertexData& vertexData = vertexDataList[index];
                            VUASSERT(vertexData.activeFaceListSize > 0, "Forsyth");
                            VUASSERT(vertexData.cachePos0 >= lruCacheSize, "Forsyth");
                        ...
                        if (faceScore > bestScore)
                        {
                            ...
                            VUASSERT(bestScore <= maxValenceScore, "Forsyth");
                ...
                VUASSERT(bestScore >= 0.f, "Forsyth");
            }
            ...
            for (uint v = 0; v < 3; ++v)
            {
				...
				std::vector<uint>::iterator begin = activeFaceList.begin() + vertexData.activeFaceListStart;
				std::vector<uint>::iterator end = begin + vertexData.activeFaceListSize;
				std::vector<uint>::iterator it = std::find(begin, end, bestFace);
                VUASSERT(it != end, "Forsyth");
                std::swap(*it, *(end-1));
                --vertexData.activeFaceListSize;
...
#endif // VU_DISABLE_VERTEX_CACHE_OPT
